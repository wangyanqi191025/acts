...
class Impl final : public DoubletSeedFinder {
 public:
  ...
  void createDoubletsImpl(const ConstSpacePointProxy2& middleSp,
    ...
    const float t0M = tM - std::sqrt(rM * rM + zM * zM) / Acts::PhysicalConstants::c;
    ...
    for (auto [indexO, xyO, zrO, varianceZO, varianceRO, timeO, varianceTO] : candidateSps.zip(
             container.xyColumn(), container.zrColumn(),
             container.varianceZColumn(), container.varianceRColumn(), container.timeColumn(), container.varianceTColumn())) {
      const float xO = xyO[0];
      const float yO = xyO[1];
      const float zO = zrO[0];
      const float rO = zrO[1];
      const float t0O = timeO - std::sqrt(rO * rO + zO * zO) / Acts::PhysicalConstants::c;
