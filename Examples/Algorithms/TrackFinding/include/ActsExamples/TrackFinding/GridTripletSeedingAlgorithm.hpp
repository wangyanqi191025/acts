...
class GridTripletSeedingAlgorithm final : public IAlgorithm {
 public:
  struct Config {
    ...
    float timeCoffMiddleBottomDoubletFinder = 5.f; // times the sigma of the time difference distribution for compatible doublets
    /// Whether to apply a time cut in the middle-top doublet finder
    bool useTimeCutMiddleTopDoubletFinder = false;
    float timeCoffMiddleTopDoubletFinder = 5.f; // times the sigma of the time difference distribution for compatible doublets
    /// Whether to apply a time cut in the triplet filter
    bool useTimeCutTripletFilter = false;
    float timeCoffTripletFilter = 15.f; // chi2 cut for the time compatibility of the triplet
